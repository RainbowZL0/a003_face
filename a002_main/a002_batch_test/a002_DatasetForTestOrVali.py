...
from a002_main.a001_utils.a001_data_dict import DatasetDictObj
...
class DatasetForTestOrVali(Dataset):
    def __init__(
            ...
            dataset_path=None,
            dataset_dict_obj: DatasetDictObj = None,  # type: ignore
    ):
        """
        ...
        if dataset_path is not None and dataset_dict_obj is None:
            ...
            self.dataset_path = dataset_dict_obj.dataset_path  # type: ignore
            self.dataset_dict_obj = dataset_dict_obj  # type: ignore
        else:
            raise ValueError(
    ...
    def __getitem__(self, idx):
        ...
        if is_positive:
            # 正例：从同一个人中抽取两张不同的图片
            person0_key = random.choice(self.dataset_dict_obj.person_keys)  # type: ignore
            img0_path, img1_path = (
                self.dataset_dict_obj.sample_two_images_from_same_person_given_mod(  # type: ignore
            ...
            person0_key, person1_key = random.sample(
                self.dataset_dict_obj.person_keys,  # type: ignore
                2,
            )
            img0_path = self.dataset_dict_obj.sample_an_image_given_mod_and_person_key(  # type: ignore
                person_key=person0_key,
                mod_choice=mod_choice_0,
            )
            img1_path = self.dataset_dict_obj.sample_an_image_given_mod_and_person_key(  # type: ignore
    ...
    def get_modality_choices_for_test_dict(self) -> dict:
        mod_0, mod_1 = (
            self.dataset_dict_obj.modalities_dict["mod_0"],  # type: ignore
            self.dataset_dict_obj.modalities_dict["mod_1"],  # type: ignore
        )
    ...
    def get_a_pair_of_random_modality_choice(self) -> tuple:
        mod_choice_list: list = random.choice(
            list(self.modality_choices_for_test_dict.values())
        )
