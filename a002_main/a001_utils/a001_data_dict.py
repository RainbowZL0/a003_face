...
class DatasetDictObj:
    def __init__(self, init_with_dataset_path=None, init_with_dict=None):
        if init_with_dict and init_with_dataset_path:
            raise ValueError(
                "请指定init_with_dict或init_with_dataset_path的其中一个，不能同时使用。"
            )
        if not init_with_dict and not init_with_dataset_path:
            raise ValueError(
                "请指定init_with_dict或init_with_dataset_path的其中一个，不能都不指定。"
            )
        ...
        if init_with_dataset_path:
            ...
            self.dataset_path = ""
            self.dataset_dict = init_with_dict if init_with_dict is not None else {}

        # 获取所有人的key, key类型为字符串
        if self.dataset_dict is not None:
            self.person_keys = list(self.dataset_dict.keys())
        else:
            raise TypeError("self.dataset_dict should not be None.")
        # 总人数
    ...
    def build_dataset_dict(self) -> dict:
        ...
        self.dataset_path: str  # type: ignore
        for root, dirs, files in os.walk(self.dataset_path):
            ...
            if (
                    root_path_parts[-1] == "images"
                    and root_path_parts[-2] in dataset_type_list
            ):
                for image_path in files:
                    ...
                    dataset_dict_0[person_key_in_dict][modality_of_this_image].append(
                        full_path
                    )
        sorted_dataset_dict = {
            key: dataset_dict_0[key] for key in sorted(dataset_dict_0)
        }
    ...
    def sample_two_images_from_same_person_given_mod(
            self, person_key, mod_choice_0, mod_choice_1
    ) -> tuple[str, str]:
        """
